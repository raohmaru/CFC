...
# Octgn.Core.DataManagers.SetManager.Get().Sets
# cardModel = Octgn.Core.DataExtensionMethods.GameExtensionMethods.GetCardByName(Octgn.Program.GameEngine.Definition, 'Zombie')
# cardModel = Octgn.Core.DataExtensionMethods.GameExtensionMethods.GetCardById(Octgn.Program.GameEngine.Definition, Guid.Parse('aa867ea1-89f8-4154-8e20-2263edd00051'))
# setModel = Octgn.Core.DataExtensionMethods.GameExtensionMethods.GetSetById(Octgn.Program.GameEngine.Definition, Guid.Parse('aa867ea1-89f8-4154-8e20-2263eddb8354'))

'''
card = Octgn.Play.Card.Find(table[0]._id)
model = Octgn.Play.CardIdentity.Find(table[0]._id)
alt = model.Model.Properties[''].Clone()
# alt = card.Properties['noability'].Clone()
prop = [p for p in alt.Properties.Keys if p.Name == "Rules"]
alt.Properties.Item[prop] = "Lorem ipsum"

model.Model.Properties['newability'] = alt



# Generate proxy
model.Model.Alternate = 'newability'

set = Octgn.Core.DataExtensionMethods.CardExtensionMethods.GetSet(model.Model)
uri = System.Uri(Path.Combine(set.ProxyPackUri, Octgn.Core.DataExtensionMethods.CardExtensionMethods.GetImageUri(model.Model) + ".png"))
files = Directory.GetFiles(set.ProxyPackUri, Octgn.Core.DataExtensionMethods.CardExtensionMethods.GetImageUri(model.Model) + ".png")
if files.Length == 0:
   cropPath = Path.Combine(Path.Combine(set.ImagePackUri, "Crops"))
   if Directory.Exists(cropPath):
       crops = Directory.GetFiles(cropPath, model.Model.ImageUri + ".jpg")
       if crops.Length == 0:
           Octgn.Core.DataExtensionMethods.GameExtensionMethods.GetCardProxyDef(Octgn.Program.GameEngine.Definition).SaveProxyImage(Octgn.Core.DataExtensionMethods.CardExtensionMethods.GetProxyMappings(model.Model), uri.LocalPath)
       else:
           Octgn.Core.DataExtensionMethods.GameExtensionMethods.GetCardProxyDef(Octgn.Program.GameEngine.Definition).SaveProxyImage(Octgn.Core.DataExtensionMethods.CardExtensionMethods.GetProxyMappings(model.Model), uri.LocalPath, crops[0])

model.Model.Alternate = ''
table[0].switchTo('newability')
'''